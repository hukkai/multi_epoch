...
class ChunkedLlamaForCausalLMBase(nn.Module):
    def __init__(
        ...
        *,
        ...
        if use_scale:
            self.chunk_scale = nn.Parameter(torch.ones(total_chunks))
    ...
    def forward(
        ...
        chunk_affine = self.chunk_affine
        if self.chunk_scale is not None:
            chunk_affine = chunk_affine + self.chunk_scale[:, None, None]
        else:
            chunk_affine = chunk_affine + 1.0
...
class ChunkedLlamaForCausalLM(ChunkedLlamaForCausalLMBase):
    def __init__(self, 
        ...
        super().__init__(
            ...
            num_matrix=2 + 3 * config.mlp_ratio,
...
def build_model(
    ...
    *,
    ...
    if orthogonal_type == "mlp":
        return ChunkedMlpLlamaForCausalLM(config, init_chunk_weights=init_chunk_weights, use_scale=use_scale)
    if orthogonal_type == "atten":
        return ChunkedAttentionLlamaForCausalLM(config, init_chunk_weights=init_chunk_weights, use_scale=use_scale)
    if orthogonal_type == "all":
        return ChunkedLlamaForCausalLM(config, init_chunk_weights=init_chunk_weights, use_scale=use_scale)
